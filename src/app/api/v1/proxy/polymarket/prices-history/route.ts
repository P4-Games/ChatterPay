...
import { POLYMARKET_CLOB_API_URL, POLYMARKET_ADAPTER_AUTH_TOKEN } from 'src/config-global'

// Server-side proxy for Polymarket's `prices-history` — keeps the adapter's
// Bearer token off the client. When `POLYMARKET_ADAPTER_AUTH_TOKEN` is set the
...
export async function GET(req: NextRequest) {
  ...
  const useAdapter = Boolean(POLYMARKET_ADAPTER_AUTH_TOKEN)
  ...
  try {
    const res = await fetch(`${baseUrl}/prices-history?${params.toString()}`, {
      headers: useAdapter ? { Authorization: `Bearer ${POLYMARKET_ADAPTER_AUTH_TOKEN}` } : {},
      cache: 'no-store'
    })
    if (!res.ok) {
      return NextResponse.json(
