...
import { resetSecurityPin } from 'src/app/api/services/security/security-service'
...
export async function POST(req: NextRequest, { params }: { params: IParams }) {
  ...
  try {
    ...
    const body = (await req.json()) as ResetPinPayload
    if (!body?.newPin) {
      return NextResponse.json(
        { ok: false, message: 'Missing newPin in request' },
        {
          status: 400,
          headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' }
        }
    ...
    if (!answers.length) {
      return NextResponse.json(
        { ok: false, message: 'Missing recovery answers in request' },
        {
          status: 400,
          headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' }
        }
    ...
    const result = await resetSecurityPin(phoneNumber, body.newPin, answers)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error resetting PIN' },
