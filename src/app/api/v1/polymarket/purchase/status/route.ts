...
import { purchaseStatus } from 'src/app/api/services/polymarket/polymarket-service'
import {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!purchase_id) {
      return NextResponse.json(
        { ok: false, message: 'Missing purchase_id' },
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      )
    }

    const result = await purchaseStatus(phoneNumber, purchase_id)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error fetching purchase status' },
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    )
  }
}
