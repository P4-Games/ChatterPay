...
import { cancelOrder } from 'src/app/api/services/polymarket/polymarket-service'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!order_id) {
      return NextResponse.json(
        { ok: false, message: 'Missing order_id parameter' },
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      )
    }

    const result = await cancelOrder(phoneNumber, order_id)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error cancelling order' },
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    )
  }
}
