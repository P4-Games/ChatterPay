...
import { getAccountStatus } from 'src/app/api/services/polymarket/polymarket-service'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await getAccountStatus(phoneNumber)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error checking account status' },
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    )
  }
}
