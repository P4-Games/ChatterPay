...
import { getPositions } from 'src/app/api/services/polymarket/polymarket-service'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await getPositions(phoneNumber)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error fetching positions' },
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    )
  }
}
