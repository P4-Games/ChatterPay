...
import { bridgeWithdraw } from 'src/app/api/services/polymarket/polymarket-service'
import {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!amount) {
      return NextResponse.json(
        { ok: false, message: 'Missing withdraw amount' },
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      )
    }

    const result = await bridgeWithdraw(phoneNumber, amount)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error processing withdraw' },
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    )
  }
}
