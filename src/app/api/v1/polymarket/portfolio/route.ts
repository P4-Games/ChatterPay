...
import { getPortfolio } from 'src/app/api/services/polymarket/polymarket-service'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await getPortfolio(phoneNumber)
    ...
    return NextResponse.json(
      { ok: false, message: 'Error fetching portfolio' },
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    )
  }
}
